...
function Navbar({ items }: NavbarProps) {
  ...
  return (
    <header className="relative px-3 py-4 md:px-4 lg:flex lg:h-[100px] lg:items-center lg:justify-between lg:gap-8 lg:px-9 lg:py-0">
      <div className="flex items-center justify-between lg:block">
        <NavLink to="/" className="inline-flex items-center" aria-label="Inicio">
          <img src={logo} alt="Psicología ula" className="h-auto w-[90px] object-contain lg:w-[105px]" />
        </NavLink>
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `border-b-2 pb-1 text-[1.07rem] font-medium no-underline transition-colors ${
                isActive ? 'border-b-[#3875c9] text-[#3875c9]' : 'border-b-transparent text-slate-800'
              }`
            }
            end={item.to === '/'}
          >
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `border-b pb-2 text-[0.98rem] font-medium no-underline transition-colors ${
                    isActive ? 'border-b-[#3875c9] text-[#3875c9]' : 'border-b-slate-200 text-slate-800'
                  }`
                }
                end={item.to === '/'}
              >
